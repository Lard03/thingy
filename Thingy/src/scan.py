...
from util.sort import insort_right
...
class MemoryScanner:
            ...
            for region in process(self.pm.process_handle):
                ...
                try:
                    ...
                    for match in re.finditer(pattern, data, re.DOTALL):
                        addr = region.start + match.span()[0]
                        addr_obj = Address(addr, search_value)
                        insort_right(self.scanned_addresses, addr_obj, key=lambda a: a.address)
                        self.update_results([f"{hex(addr_obj.address)} -> {value}"])
    ...
    def next_scan_memory(self, value, value_type: Type):
        ...
        try:
            ...
            self.update_results([f"{hex(addr_obj.address)} -> {value}" for addr_obj in self.scanned_addresses])
    ...
    def edit_address(self, index, new_value):
        if 0 <= index < len(self.scanned_addresses):
            addr_obj = self.scanned_addresses[index]
            try:
                self.pm.write_bytes(addr_obj.address, new_value, len(new_value))
                addr_obj.value = new_value
                self.update_status(f"Address {hex(addr_obj.address)} updated")
            except PymemError as e:
